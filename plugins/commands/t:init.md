Check and set up the prerequisite tools and project state required by claude-code-utils plugins, plus optional Telegram notifications when Claude needs you. Detection is handled by deterministic shell scripts; this command runs them, presents results, and handles the interactive parts (asking the user, applying installs, summarizing).
