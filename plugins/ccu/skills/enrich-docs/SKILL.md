...
**Prose style is owned by the `tech-doc` skill.** Read [`plugins/ccu/skills/tech-doc/SKILL.md`](../tech-doc/SKILL.md) before writing a single sentence and follow it for every sentence you write here. It defines the banned patterns (history, process narration, AI voice), the sentence-level rules, and the per-doc-type structure. This workflow owns *what* to document; that skill owns *how it reads*. Delegate to it with `doc-type: module` (or `api`/`design`, whichever fits the gap) and `constraints: add only, never delete existing docs`.
