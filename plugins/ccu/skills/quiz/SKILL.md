...
Comprehension gate for code produced without the user watching — an autonomous `/auto` or `/orchestrator` run, a long agent session, or any branch the user is about to merge unread. Produce an HTML explainer of the change set with a quiz at the bottom, then grade the user's answers in conversation. The user merges only what they can pass a quiz on.
