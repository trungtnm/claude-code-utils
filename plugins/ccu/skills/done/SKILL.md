...
0.5. **Audit decisions** — If significant work was done this session (beads completed, features implemented), review the session for undocumented technology, schema, API, or architecture choices and record each one (ADR-gate decisions → `docs/adr/NNNN-slug.md` + a one-line `.ccu/DECISIONS.md` pointer; the rest → a journal entry).
