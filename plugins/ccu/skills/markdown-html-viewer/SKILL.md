...
first and use the current Codex user-input mechanism wherever this workflow
...
**Visual identity — "The Warm Reference Desk" (see `DESIGN.md`).** The shell follows one design system: warm paper (`#fbfaf9`) under near-black warm ink, a single **terracotta** accent (`#b0512f`) for all wayfinding, green/amber/red used strictly as *status* (callouts), and a flat surface carried by 1px hairlines — no drop shadows. Code blocks sit on a near-black **Espresso** surface in both themes with a corner language label; prose is capped near a 74ch reading column while tables and code scroll wider inside their own box; callouts are full-bordered tinted boxes (never a `border-left` stripe). Dark mode is an on-brand warm-espresso counterpart (DESIGN.md itself is light-only). All theming flows through CSS variables in `:root` / `[data-theme="dark"]`, so palette tweaks live in two blocks. **Mermaid diagrams are restyled, not just recolored:** rounded nodes and subgraph cards, a warm sans font, hairline warm-gray edges, and — echoing DESIGN's sequence language — dashed return arrows (`-->>`) rendered in terracotta while solid requests stay neutral ink. This lives in `mermaidThemeVars()` (colors, per-theme) + the `mermaid.initialize` config (font/curve/spacing — must be config, since Mermaid measures layout from the font) + a `.content pre.mermaid svg …` CSS block (shape/stroke/weight, via theme vars so it tracks light/dark).
