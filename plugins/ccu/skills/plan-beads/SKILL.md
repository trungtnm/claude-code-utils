...
before starting. Treat the Claude `Task(...)`, `Skill(...)`, and
`AskUserQuestion` syntax below as semantic instructions and use Codex's current
collaboration, skill, planning, and user-input tools. Claude Code should use the
native examples as written.

## IMPORTANT: Execution Mode

**This skill replaces the host's built-in planning.** When this skill is active:
