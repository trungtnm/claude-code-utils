---
name: demo-to-prod
description: Convert a working demo/prototype UI into a production application with a stubbed backend and beads for wiring
argument-hint: [demo path or app name]
---
...
10. **Next steps** — Report to the user:
    ...
    Run `/auto` to let agents work through the backend beads.
    Or run `/onboard` first to set up the development environment.
    ```
...
- `--auto`: after conversion, immediately start `/auto` for backend beads
