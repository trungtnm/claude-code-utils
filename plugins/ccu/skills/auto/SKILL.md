...
6. **Loop** — re-check with `br show <id> --json` (already closed → move on, not an error), close with `br close <id> --reason "..."`, `release_file_reservations()`, and sync with `br sync --flush-only`. Read your inbox, handle anything that needs a reply, and return to step 3.
...
7. **Wrap up** — when no actionable work remains:
