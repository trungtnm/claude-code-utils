...
```bash
herdr agent start <name> --cwd <repo-root> -- claude "<bootstrap prompt>"
```
...
- `herdr pane run <pane_id> <command>` exists for command-plus-Enter in one call — meant for shell panes; for messaging a Claude peer, prefer `agent send` + `send-keys Enter` so the text lands in the agent's input box exactly as written.
...
| Label missing from `agent list` | Pane died | Reconcile by evidence precedence; respawn `claude --continue` if work unfinished |
