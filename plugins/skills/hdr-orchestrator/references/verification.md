...
3. Ledger: status `rejected`, rejection count +1. If the pane is gone, respawn with `claude --continue` at the repo root — it re-reads the brief and the rejection file.
...
- **Reviewer verdict:** the `Task(subagent_type="reviewer")` findings feed this same reject path.
