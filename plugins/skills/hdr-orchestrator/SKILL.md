...
description: Turn this session into the repo's delegation coordinator — spawn peer Claude Code sessions in visible Herdr terminal panes, one per task, with bead-authoritative briefs, lock-free admission control, and evidence-based verification. Use when delegating tasks to peer sessions, running ad-hoc parallel work in panes, coordinating main-agent-to-main-agent work via herdr, or when asked to "delegate this", "spawn a peer for X", or "run these in parallel panes".
---

# hdr-orchestrator: Main-Agent Delegation Through Herdr Panes

This skill turns the current session into the **coordinator** for this repo. Peers are ordinary Claude Code main-agent sessions spawned in Herdr terminal panes — visible, attachable, persistent — working in the **same tree on the current branch**. Coordination signals (agent state, screen reads) are shell operations against the `herdr` CLI, free of model tokens.
...
Run all three checks before anything else. If any fails, **refuse with instructions — never half-run**:
...
- **On blocked:** `herdr agent read <label> --source recent` to see the actual question. If it falls **within the brief's granted authority**, answer straight into the pane (`herdr agent send` + Enter — see the driving manual). Anything touching plan, scope, or irreversible actions **escalates to the user** (AskUserQuestion → the existing Telegram hook). Ledger stays `blocked` — never silently dropped.
...
The file makes rejection **idempotent**: if the pane died, `claude --continue` at the repo root re-reads it. Ledger increments the rejection count.
...
Tiered runs go **Architect → Coder → Reviewer** — a temporal handoff, two panes per delegation maximum, sequential. Full protocol (split test ownership, settings-deny enforcement, assertion-line rejection rule, change-request routing, Reviewer as `Task()` subagent): [references/tiered-execution.md](references/tiered-execution.md).
...
| Peer pane dies mid-task | `herdr agent list` lacks the label | Reconcile by evidence precedence: `RESULT.md` or closed bead → verify as done; else respawn with `claude --continue` at the repo root (re-reads brief + any REJECTION file) |
