...
Written by the coordinator to `.ccu/delegations/<id>/REJECTION-<n>.md` — **before** any keystroke is injected. The file is the durable order; the injected message only points at it. If the pane died, a respawn (`claude --continue` at the repo root) re-reads this file — the rejection never depends on a keystroke having landed.
