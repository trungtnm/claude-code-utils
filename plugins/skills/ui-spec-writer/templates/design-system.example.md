...
- Agents default to `shadow-md` or `shadow-lg` — Apple uses a single custom soft shadow or none; reject Tailwind's default shadows.
- Agents add secondary accent colors (green for success, red for error) — Apple restricts the palette to neutrals + blue; status is communicated through copy and icons, not hue.
- Agents use `rounded-md` (6px) for buttons — Apple's button radius is 8px, and CTAs are often `rounded-full` (980px pill).
- Agents use `font-bold` (700) for emphasis — Apple reaches for 600 (semibold); 700 is reserved for specific card titles.
- Agents leave tracking at default — Apple applies negative letter-spacing at every size including body; verify `tracking-tight` or custom tracking on all text.

## 10. Changelog

- `v1` — initial extraction (example document; not derived from a live repo)
